...
public class SparseMat implements Matrix {

    public SparseMat(String fileName) throws IOException {
        if (fileName != null) {
    ...
    public Matrix mul(Matrix bb) throws IOException {
        SparseMat a = this;
        SparseMat res = null;
        if (bb instanceof SparseMat) {
            SparseMat b = (SparseMat) bb;
            res = a.mulSS(b);
        }
        else {
            DenseMat b = (DenseMat) bb;
            b=DToS(b);
            res = a.mulSS(b);
        }
    ...
    private SparseMat mulSS(SparseMat b) throws IOException {
        SparseMat a = this;
        SparseMat c = new SparseMat(null);
        b = b.transpose(b);
        ...
        for (int stroka = 0; stroka < a.pointersArr.length - 1; stroka++) {
            for (int stolb = 0; stolb < b.pointersArr.length - 1; stolb++) {
                SparseMat v1 = new SparseMat(null);
                ...
                SparseMat v2 = new SparseMat(null);
    ...
    private SparseMat pmulSS(SparseMat m2) throws InterruptedException, IOException {
        Dispatch d = new Dispatch();
        SparseMat m1 = this;
        SparseMat res = new SparseMat(null);
        m2 = m2.transpose(m2);
        ArrayList<Double> values = new ArrayList<>();
        ArrayList<Integer> cols = new ArrayList<>();
        ArrayList<Integer> pointers = new ArrayList<>();
    ...
    public static void printf(SparseMat a) {
        printf(a.valuesArr);
        printf(a.colsArr);
    ...
    private double scalarMul(SparseMat b) {
        SparseMat a = this;
    ...
    public static SparseMat DToS(double[][] mas) throws IOException {
        SparseMat a = new SparseMat(null);
    ...
    public static void fillArrays(SparseMat a, String fileName) throws IOException {
    ...
    public SparseMat transpose(SparseMat a) throws IOException {
        int j;
        double v;
        SparseMat newA = new SparseMat(null);
    ...
    public boolean equals(Object bm) {
        boolean ans;
        if (bm instanceof SparseMat) {
            SparseMat b = (SparseMat) bm;
            SparseMat a = this;
            boolean ans1 = true;
